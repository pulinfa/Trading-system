...
func main() {
	...
	cid := argsfunc() //cid是登录的客户号，t是间隔的时间
	...
	for i := 0; i < 20; i++ {
		...
		if inOrOut != 3 && inOrOut != 4 {
			//随机一个持仓进行更改
			idx := random(0, len(MyHoldingRecords))

			ET = MyHoldingRecords[idx].exchangeType
			SC = MyHoldingRecords[idx].stockCode
			num = int32(random(0, 100))
		}

		ti := time.Now().UnixMicro() //秒的时间
		...
		ti = time.Now().UnixMicro() - ti //获取查询响应时间
	...
	fmt.Println("avg response time: ", sum/20)
}

func argsfunc() string {
	fmt.Println(("argsfunc start"))
	tmp, _ := strconv.Atoi(os.Args[1])
	cid := fmt.Sprintf("%012d", tmp)
